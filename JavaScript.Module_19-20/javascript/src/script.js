...
var skills = _.map(objJson,'skills');
var skillsFlatten = _.flattenDeep(skills);
var skillsFlattenUniq = _.uniq(skillsFlatten);
var skillsFlattenUniqSort = _.sortBy(skillsFlattenUniq, _.toLower);

console.log('Уникальные скиллы: ', skillsFlattenUniqSort );

var nameHuman = _.map(objJson, function(o) {
    return { "name": o.name, "friends": o.friends}
  });

var nameHumanSort = _.map(_.sortBy(nameHuman, 'friends.length'), 'name');

console.log('У них больше всего друзей! ', nameHumanSort );

var nameFriends = _.map(objJson, function(o) {
  return {"name": _.map(o.friends, "name")}
});
var nameFriendsMap = _.map( nameFriends, 'name');
var nameFriendsFlatten = _.flattenDeep( nameFriendsMap );
var nameFriendsUniq = _.uniq(nameFriendsFlatten);
var nameFriendsSort = _.sortBy(nameFriendsUniq);

console.log('Уникальные друзья всех друзей: ', nameFriendsSort );
